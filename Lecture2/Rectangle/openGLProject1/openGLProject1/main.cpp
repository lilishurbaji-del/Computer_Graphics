...
// وظيفته: تحديد لون البكسلات (هنا زهري)
...
int main()
{
	...
	// مستطيل يتكون من مثلثين: 6 رؤوس
	float vertices[] = {
		// المثلث الأول
		-0.8f, -0.3f, 0.0f,  // 0: يسار أسفل
		 0.8f, -0.3f, 0.0f,  // 1: يمين أسفل
		-0.8f,  0.3f, 0.0f,  // 2: يسار أعلى

		// المثلث الثاني
		 0.8f, -0.3f, 0.0f,  // 3: يمين أسفل
		 0.8f,  0.3f, 0.0f,  // 4: يمين أعلى
		-0.8f,  0.3f, 0.0f   // 5: يسار أعلى
	};
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// ب. التنظيف (لون الخلفية) - رمادي
		glClearColor(0.3f, 0.3f, 0.3f, 1.0f); // لون رمادي
		glClear(GL_COLOR_BUFFER_BIT);

		// ج. الرسم
		glUseProgram(shaderProgram); // تفعيل الشيدر
		glBindVertexArray(VAO); // تفعيل كائن المستطيل
		glDrawArrays(GL_TRIANGLES, 0, 6); // ارسم 6 رؤوس (مثلثان)
